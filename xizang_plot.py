...
# 读取 CSV 文件，指定第二行作为表头
df = pd.read_csv('xizang_land_use.csv', header=1)
import os


title = pd.read_csv('xizang_land_use.csv').columns[0]
...
# 绘制每一列的数据，并保存为 xizang_land_use_i.png
for i, column in enumerate(df.columns[1:len(df.columns)-1], start=1):
    ...
    # 保存图像到本地，文件名为 xizang_land_use_i.png
    plt.savefig(f'{pic_folder}/xizang_land_use_{i}.png', dpi=300)
